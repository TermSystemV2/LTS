...
async def get_coursers_by_term_pass(queryItem: schemas.ClassQuery, db: Session = Depends(get_db),
    ...
    if state == 0:
        ...
        courseNameList = [courseName for courseName in courseNameList if courseName not in config.NOT_SHOW_COURSENAME]
        for courseName in courseNameList:
            ...
            res = db.query(models.Scores).filter(
                ...
                with_entities(models.Scores.grade, models.Scores.score).all()
            ...
            sum_students = 0
            for grade in gradeList:
                sum_students += sum(tmpDict["gradeDistribute"][str(grade)].values())
