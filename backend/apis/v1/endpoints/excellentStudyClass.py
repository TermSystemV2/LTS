...
async def get_excellentStudyClass_info_histogram(db=Depends(get_db),redis_store=Depends(course_cache)):
    ...
    if state != 0:
        ...
        if len(resp_data) != 0:
            # 写入 redis
            try:
                await redis_store.setex("excellentStudyClassHist", config.FAILED_STUID_INFO_REDIS_CACHE_EXPIRES,
...
async def get_excellentStudyClass_info_line(db=Depends(get_db),redis_store=Depends(course_cache)):
    ...
    if state != 0:
        ...
        if len(resp_data) != 0:
            # 写入 redis
            try:
                await redis_store.setex("excellentStudyClassLine", config.FAILED_STUID_INFO_REDIS_CACHE_EXPIRES,
                                json.dumps(resp_data))
            except Exception as E:
