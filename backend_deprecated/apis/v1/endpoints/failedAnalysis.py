    ...
    try:
        ...
        if state != 0:
            ...
            class_list = (
                ...
                .distinct()
                .order_by(models.Student.stuClass)
                .all()
            )
            dict_list = []
            i = 1
            ...
            if len(dict_list) != 0:
                ...
                await redis_store.setex(
                    "class_info", config.REDIS_CACHE_EXPIRES, resp_json
                )
...
async def get_classes_fun(db: Session, redis_store: Redis):
    ...
    try:
        ...
        if state != 0:
            ...
            resp_json = json.dumps(dict_list, ensure_ascii=False)
            # 将信息存入 redis
            await redis_store.setex("class_info", config.REDIS_CACHE_EXPIRES, resp_json)
