...
async def get_stuAnalysis(request:Request,select_Bar:schemas.StuAnalysisBar,db=Depends(get_db),redis_store:Redis=Depends(course_cache)):
    """
    ...
    stuAnsInfos = await getStuAnsFromSql(db,redis_store,select_term_bar, select_class_bar)
    if stuAnsInfos != -1:
        return Response200(data=stuAnsInfos)
    else:
        return Response400(code=status.HTTP_404_NOT_FOUND,msg='所选班级信息暂未导入，请导入后操作')


@failA_router.get("/stuclasses")
async def get_classes(db:Session=Depends(get_db),redis_store:Redis = Depends(course_cache)):
    """
    获取班级信息
    :param db:
    :return:
    ...
    try:
        ...
        if state != 0:
            ...
            class_list = db.query(models.Student).with_entities(models.Student.stuClass).distinct().order_by(models.Student.stuClass).all()
            dict_list = []
            i = 1
            ...
            if len(dict_list) != 0:
                ...
                # 将信息存入 redis
                await redis_store.setex("class_info",config.CLASS_INFO_REDIS_CACHE_EXPIRES,resp_json)

                return Response200(data=dict_list)
            else:
                return Response400(msg="没有查询到相关数据，请检查查询关键字 or 数据库数据是否为空")
    except Exception as e:
        return Response400(code=status.HTTP_500_INTERNAL_SERVER_ERROR,msg="发生异常：" + str(e))
...
async def getStuAnsFromSql(db:Session,redis_store:Redis,select_term_bar,select_class_bar):
    ...
    try:
        ...
        if className not in stuClass_ann_list:
            print("班级信息未导入")
            return -1
        stuAnalysisinfos = db.query(models.StuAnalysis).filter(models.StuAnalysis.term == term,
                                                    models.StuAnalysis.stuClass == className).all()
        # print("get stuanakysis: ",stuAnalysisinfos)
    except Exception as E:
        logging.error("数据库异常" + str(E))
        return Response400(code=status.HTTP_500_INTERNAL_SERVER_ERROR,msg="数据库异常" + str(E))
    stuAna_list = []
...
async def getClass(index,db:Session,redis_store:Redis):
    ...
    if data and index > 0:
        ...
        # 不在缓存中。从数据库中去查找第一个班级
        try:
            data = db.query(models.Student).with_entities(models.Student.stuClass).distinct().order_by(
                models.Student.stuClass).first()
            return data[0]
        except Exception as E:
